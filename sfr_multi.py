...
class Simulation:
    def __init__(self,num_sphere = 30) -> None:
...
num_agents = 3
robot = sfr().to(device)
actions = torch.zeros(num_agents, 9).to(device)
sol = robot.odeStepFull(actions) 

print(sol.shape)  # (timesteps, num_agents, 14)


offsets = [np.array([i * 0.2, 0, 0]) for i in range(num_agents)]
sim = Simulation(num_sphere=30)
sim.create_robot(num_agents=num_agents)
sim.reset()
...
while sim.simulation_app.is_running():   
    if sim.my_world.is_playing():    
        ...
        t = sim.my_world.current_time
        w = 2 * np.pi
        actions = torch.zeros((num_agents, 9), device=device)
        sine_wave = torch.sin(w * t + torch.arange(num_agents, device=device) * 0.2)
        actions[:, 1] = 0.005 * sine_wave
        actions[:, 4] = 0.005 * sine_wave
        actions[:, 7] = 0.005 * sine_wave

        sol = robot.odeStepFull(actions)  # (timesteps, num_agents, 14)
        sol = robot.downsample_simple(sol, sim.num_sphere).detach().cpu().numpy()  # (num_sphere, num_agents, 14)

        # Gather all positions into one (N*agents, 3) array
        positions = []
        for agent_id in range(num_agents):
            agent_offset = np.array([0.2 * agent_id, 0, 0])
            agent_positions = sol[:, agent_id, :3] + agent_offset
            positions.append(agent_positions)
        positions = np.vstack(positions)  # shape: (num_agents*num_sphere, 3)

        objects = [sim.my_world.scene.get_object(name) for name in sim.sphere_paths]
